...
- The first directly downloadable Toven binary release pipeline: `.github/workflows/release.yml` builds a real per-target `toven` archive for every supported target (cross-compiling `aarch64-unknown-linux-gnu` through `cross`), assembles a combined `SHA256SUMS` and its keyless Sigstore/cosign signature, a CycloneDX SBOM, and GitHub build provenance, then runs `toven release publish` behind the protected, manually approved `release` environment and verifies every published archive by checksum and, where the runner can execute it, by running `--version`. `toven.toml`'s `[ecosystems.rust.release.host]` now declares the fixed, version-free `dist/` asset paths this pipeline produces. `scripts/package-release-binary.sh` and `scripts/verify-release-binary.sh` are the shared packaging/verification steps; `make release-artifacts` now packages a real native-target archive instead of a source tarball.
