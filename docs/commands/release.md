...
`bump` and `tag`/`publish` share one manifest-mutation prefix through the `ManifestMutator` phase contract, so the versions a `bump` commit carries are exactly the ones a later `tag` would produce. The same version-input flags apply (`--patch`/`--minor`/`--major`, `--set-version`, `--pre`, `--base`), and `bump` obeys every flow guarantee: it requires `--yes`, checks the allowed branch and a clean worktree before mutating, and `--dry-run` previews the planned version transitions and the changelog paths that would roll without writing anything. `--no-commit` is valid only on `bump`; `--no-push` is not (a bump never pushes).
