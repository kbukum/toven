...
Tasks authored `cacheable = false` are never cached either: a mutating `*-fix` task (Go's `format` / `tidy-fix`) must apply its change on every run, so it can never be satisfied by a stale content-key hit.
