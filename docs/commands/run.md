...
| `tiles` | In-terminal live tiles: one region per in-flight unit that grows with its output — a single header line for a silent or fast unit up to a bounded tail — collapsing to a verdict on completion. A failing unit replays its tail inline and is re-surfaced in a consolidated `failures (N)` section directly above the run summary, so failures are never buried. |
