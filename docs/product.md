...
The release is modeled as a **flow**: an ordered set of phases — select, bump, tag, package, sign, publish, host, image, provenance — that the engine orchestrates. A service shipped as a container image is a first-class releasable module: the `image` phase builds it once, pushes it to a primary registry plus mirrors, and cosign-signs the digest; the `provenance` phase then attests SLSA build provenance over exactly what was published.
