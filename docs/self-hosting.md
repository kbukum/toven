...
4. Assert the version the engine will cut equals `v${Cargo.toml}` before any binary is built (`make release-plan-version-check`). Toven's release binaries are built and packaged from the checked-out `Cargo.toml` in the `build` job, before the `publish` job creates the tag, so each packaged `toven --version` reports the `Cargo.toml` version. If a stray tag (one left at or above the declared version by a failed run flips the release baseline off `initial`), a forgotten version bump, or an explicit override makes the engine plan a different version, the published tag and the built binaries diverge and every post-publish Verify job fails with an opaque `release not found`. The preflight fails closed in the mutation-free preview with the exact divergence and its fixes, long before anything is built or the publish gate is approved.
