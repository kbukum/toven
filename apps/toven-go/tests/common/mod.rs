...
//!
//! Included via `mod common;` by each repo-driven `tests/smoke_*.rs`. Unlike the
//! `__serve` federation smoke (which deliberately avoids the `go` toolchain), the
//! repo-driven flows discover and apply real Go modules, so every one is gated on
//! a `go` toolchain being present via [`go_available`].
#![allow(dead_code)]
...
/// Whether a `go` toolchain is discoverable on `PATH`. Go discovery shells out to
/// `go`, so repo-driven smokes skip (stay green) when it is absent.
