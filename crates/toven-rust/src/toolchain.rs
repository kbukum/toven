...
///
/// Discovery only stamps `Workspace.toolchain.tool = "cargo"`; this spec is
/// executed after affected-filtering, and its trimmed stdout — e.g. `"cargo
/// 1.94.0 (… 2026-03-24)"` — becomes the cache-significant version string. The
/// companion `rustc --version` identity is composed by the engine's toolchain
/// resolution on top of this spec; the port carries a single probe.
#[must_use]
