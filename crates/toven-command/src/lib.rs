...
//!   [`CommandConfig`] and bakes a [`CommandAdapter`]. There is no convention
//!   to auto-detect, so its wizard detection returns `None`.
...
//!   `[tasks.*]` argv via `common().tasks` — no built-in build/test/lint
//!   defaults are invented. Its toolchain probe is the declared `[toolchain]`
//!   if present, else derived from the first declared task's program; release
//!   is out of scope, so
...
// See the `toven-rust` lib note: `pub(crate)` helpers shared across sibling modules trip the
// `redundant_pub_crate` (nursery) lint, which conflicts with `unreachable_pub`. Allow the nursery
// lint crate-wide and keep honest `pub(crate)` visibility (the structure guard forbids per-`mod.rs`
// attributes).
