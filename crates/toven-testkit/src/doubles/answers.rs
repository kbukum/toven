...
//! [`Questionnaire`] deterministically without a terminal. By default it answers
//! every questionnaire with an empty [`Answers`] set (matching an adapter that
//! asks nothing); per-ecosystem scripted answers override that, and the double
//! records every questionnaire it was asked for post-hoc assertions.
...
///
/// Ecosystems without a scripted entry resolve to an empty answer set unless the
/// double was put in strict mode with [`strict`](Self::strict), in which case an
/// unscripted questionnaire is a hard error (modeling a missing prompt).
...
impl ScriptedAnswers {
    /// Construct a provider that answers every questionnaire with empty answers.
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    }
