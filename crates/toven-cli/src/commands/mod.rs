...
//! projections in [`introspect`]; the cache-maintenance verbs in [`cache`]; and
//! the verbs whose behavior lands in later steps ([`generate`], [`driver`]) are
//! wired here as typed "not yet implemented" stubs so the surface is complete.
