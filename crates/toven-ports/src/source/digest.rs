...
/// hashed content changes. The string format is an adapter detail — callers must
/// treat it as opaque and compare it only for equality, never parse it or assume
/// a particular encoding (the filesystem adapter emits a hex hash; test doubles
/// may emit anything stable). A missing path hashes to a stable empty identity
/// rather than erroring, so an absent optional shared input does not abort PLAN.
/// Hashing is a filesystem side effect, so it is an injected port: the planner
/// stays pure and tests substitute a deterministic digest while the concrete
/// filesystem adapter lives in the engine.
