...
/// erroring, so an absent optional shared input does not abort PLAN. Hashing is a
/// filesystem side effect, so it is an injected port: the planner stays pure and
/// tests substitute a deterministic digest while the concrete filesystem adapter
/// lives in the engine.
