...
use super::{ManifestMutator, Packager, Publisher, SbomProducer, TagGrammar, VersionSource};
...
///
/// The blanket implementation makes any type that satisfies all six per-phase
/// contracts a `ReleaseAdapter`, so an ecosystem adapter (e.g. the cargo or Go
/// target) implements the phase traits it needs and gains the composite for
/// free.
pub trait ReleaseAdapter:
    VersionSource + TagGrammar + Packager + ManifestMutator + Publisher + SbomProducer
{
}

impl<T> ReleaseAdapter for T where
    T: VersionSource + TagGrammar + Packager + ManifestMutator + Publisher + SbomProducer
{
}
