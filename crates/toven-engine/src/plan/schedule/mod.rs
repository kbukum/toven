//! Schedule: relax edges by `RunStrategy`, level into federated waves, group
//! the active modules by intrinsic `FanOut`, and render one [`PlannedUnit`] per
//! group.
//!
//! Per-module `RunStrategy` decides whether a module's **intra-ecosystem**
//! ordering edges are kept (`leaf-to-top`) or dropped (`unordered`);
//! **cross-ecosystem overlay edges are never dropped**. The residual active
...
//!   the topo-levelled waves plus the per-module dependency layer derived from
//!   them.
//! - [`task`] — resolve each active module's effective
