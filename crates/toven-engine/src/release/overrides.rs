...
//! default**); the config side never rewrites user argv, and every override is
//! validated at the CLI boundary before it reaches the bump planner.
...
///
/// Built at the CLI boundary from the parsed argv; a conflicting combination
/// (a module in two level flags, or in both a level flag and `--set-version`)
/// is rejected as a typed error at construction time.
...
impl BumpOverrides {
    ...
    pub(super) fn module_level(&self, module: &ModuleRef) -> Option<BumpLevel> {
        self.module_levels.get(module).copied()
    }

    /// The pinned version for `module`, if any.
    pub(super) fn set_version(&self, module: &ModuleRef) -> Option<&Version> {
    ...
    ///
    /// # Errors
    /// Rejects an override naming a module absent from the release scope (either
    /// unknown or unchanged), so a typo can never silently no-op.
