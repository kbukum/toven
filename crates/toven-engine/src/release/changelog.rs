...
///
/// The entry's `breaking` flag stays `false` here: [`ChangeRecord`] carries only
/// a path and status, so a conventional-commit breaking signal has no source at
/// this seam. Breaking is instead driven by the explicit `--minor`/`--major`
/// argv or a per-module config `level`; this is the hook where a
/// commit-message-aware classifier would set it.
#[must_use]
