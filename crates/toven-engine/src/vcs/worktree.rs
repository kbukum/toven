...
//! unstaged + untracked) onto repo-relative [`ChangeRecord`]s — path-centric,
//! for the affected `committed ∪ worktree` union and the clean-tree guardrail.
//! `restore_worktree` composes [`Resetter::reset`](rskit_git::Resetter) +
//! [`CheckoutManager::checkout_files`](rskit_git::CheckoutManager) to roll
//! tracked files back to `HEAD` when a release apply fails before the commit.
...
/// untracked entry is `Added` and everything else is `Modified` — sufficient
/// for the path-centric affected union.
...
/// already-committed manifests, so this is the complete undo for that case.
/// Untracked files — and any first-time, not-yet-committed manifest — are
/// intentionally left in place.
pub(super) fn restore_worktree(repo: &Repo) -> AppResult<()> {
    repo.reset("HEAD", ResetMode::Mixed)?;
    // Read status *after* the reset: tracked-committed files now show as dirty and
    // get restored, while any first-time staged file is now untracked and is
