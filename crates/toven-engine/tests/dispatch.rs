...
use toven_engine::config::{dispatch, load};
...
fn loaded_ecosystem_is_configurable() {
    ...
    let document = assert_ok(load(&path, &loaded(&["rust"]), &canonical()));

    let result = assert_ok(dispatch(&document, &loaded(&["rust"]), &canonical()));
