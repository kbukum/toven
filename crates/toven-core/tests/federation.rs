...
//! runs the engine's [`serve`](toven_core::federation::serve) loop on a
//! thread connected by OS pipes, so a
//! [`RemoteAdapter`](toven_core::federation::RemoteAdapter) round-trips
...
use toven_core::federation::protocol::{
...
use toven_core::federation::resolve::{PathDriverLocator, resolve_adapters};
use toven_core::plan::dependency_graph;
...
/// in-proc ecosystems.
fn load_document(rel: &str, loaded: &[&str]) -> toven_core::config::Document {
...
impl ServeDouble {
        ...
        F: FnOnce() -> Vec<Box<dyn Provider>> + Send + 'static,
    {
        ...
        let handle = thread::spawn(move || {
            ...
            toven_core::federation::serve(&refs, driver_in, driver_out)
        });
...
fn wizard_exchange_round_trips_over_the_framed_transport() {
    ...
    let join: JoinHandle<()> = thread::spawn(move || {
        ...
        toven_core::federation::serve_wizard(&providers, driver_in, driver_out)
            .expect("serve_wizard completes");
    });

    let answers = ScriptedAnswers::new();
    let fragments = toven_core::federation::wizard_io(
        umbrella_reader,
        umbrella_writer,
        "toven-go",
...
fn wizard_driver_detect_failure_surfaces_as_a_typed_error() {
    ...
    let answers = ScriptedAnswers::new();
    let error = toven_core::federation::wizard_io(
        umbrella_reader,
        umbrella_writer,
        "toven-go",
        std::path::Path::new("/repo"),
