...
mod tests {
    ...
    fn file_at_ref_reads_a_blob_at_a_tag_and_reports_absent_paths_and_bad_refs() {
        ...
        let scenario = GitScenario::init(workspace.path()).expect("git init");
        scenario
            .commit_file("Cargo.toml", "version = \"0.2.0\"\n", "tagged state")
            .expect("commit");
        scenario.tag_lightweight("v1.0.0").expect("tag");
        // Change the file after the tag so the tagged content differs from HEAD.
        scenario
            .commit_file("Cargo.toml", "version = \"0.3.0\"\n", "post-tag change")
            .expect("commit");
        let vcs = RskitGitVcs::open(workspace.path()).expect("open");
