//! `toven-engine-release` — the release PLAN/APPLY tail.
//!
//! Layer 2b of the hexagonal architecture, a peer of [`toven-engine`] sitting
//! on the shared PLAN foundation ([`toven_engine_core`]): it owns the
...
//! federation that [`toven_engine_core`] provides. Layering stays
//! downward-only: this crate depends on [`toven_engine_core`] and never on
