...
//! `"go"`:
//!
//! - [`GoProvider`] parses `[ecosystems.go]` into a typed [`GoConfig`] and
//!   bakes a [`GoAdapter`]; it also drives the config-less init wizard for root
//!   `go.mod` projects.
//! - [`GoAdapter`] implements discovery (via `go mod edit -json` / `go work
//!   edit -json`), the toolchain probe, and run-strategy defaults. The runnable
//!   task table lives in `common().tasks`, authored by init or explicit config.
//!   Go modules release as VCS tags, so
//!   [`release_target`](toven_ports::ConfiguredAdapter::release_target) returns
