...
`/apply-step` implements exactly one step of a plan folder (from `/create-plan`). It is the unit of work that `/apply-plan` calls per step, and it can also be run directly on a single step file.
...
- **Validate** the affected crates with the `/validate` command (`cargo -p` + `make structure`), deterministic and green. A step does not land red.
- **Review** the step's diff with the relevant `/review` passes (structure/placement, rskit reuse, principles, quality, tests, docs, comments) — ideally in a fresh agent spawned via the Agent tool.
...
Work on a branch (`/create-branch` command), leave edits **uncommitted** for the maintainer to commit and push; open a PR only when explicitly asked. When that change becomes a branch/PR, name it by the change — never `step-2` or a plan/batch number.
