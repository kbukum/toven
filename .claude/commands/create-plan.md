...
A plan is a written contract for a change set: what to do, in what order, and how you will know each part is done. In this repo a plan is **not prose to admire** — it is a folder of markdown that the `/apply-plan` / `/apply-step` commands execute iteratively.
...
Numbering orders the plan; it is **internal to the plan folder only**. When a step becomes a branch/PR, name that branch/PR by the change (see the `/create-branch` command) — never `step-3` or `batch-N`.
...
A plan may **not** invent a lighter standard than Toven's. Its cross-cutting rules restate — and link to — the engineering baseline in [`docs/engineering.md`](docs/engineering.md) and defer detailed judgment to the `/review` command's eight passes. In every plan's README, make these load-bearing:
...
Creating the plan is a docs-only act under `tmp/` — no source edits, no branch, no commit. Apply it later with the `/apply-plan` command (whole plan) or `/apply-step` (one step).
