#!/usr/bin/env bash
# Benchmark case: compare Toven orchestration against native Cargo/nextest over
# the vendored rskit workspace.
#
...
#
# Every Toven approach drives the installed `toven` binary against the rskit
# repository's own `toven.toml`. Changed-module selection is the global
# `--base <ref>` baseline (there is no `--affected` flag); `toven <task>`
# without a baseline plans every module.
...
clean_cargo_target() {
  rm -rf "$BENCH_RUN_DIR/cargo-target/$BENCH_SCENARIO/$BENCH_APPROACH"
}
...
run_toven_nextest_changed() {
  (
    cd "$TARGET_REPO"
    CARGO_TARGET_DIR="$BENCH_RUN_DIR/cargo-target/$BENCH_SCENARIO/$BENCH_APPROACH" \
      "$TOVEN_BIN" nextest --base "$BASE_REF" --output jsonl
  )
}
