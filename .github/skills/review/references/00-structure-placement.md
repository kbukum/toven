...
- **`mod.rs` is declare-only.** Declarations / re-exports only — no logic, no private items. Gated by `make structure`; run it. Logic in a `mod.rs` is a structure violation (blocker).
