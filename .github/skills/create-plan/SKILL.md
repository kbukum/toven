...
- **Reuse rskit first.** Reuse or enhance the canonical rskit owner before writing a shared concern; if rskit is inadequate, improve it generically — never fork a Toven-specific copy.
...
- **Structure & placement.** Downward-only layering (L0→L1→L2→L3); a port trait in `toven-ports`, its adapter in the consuming crate, one shared double per port in `toven-testkit`; `mod.rs` declare-only.
