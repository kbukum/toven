...
- **Validate the affected crates** with the `validate` skill (`cargo -p` + `make`, scoped) — do not proceed to the next step on a red one. Ensure the submodule is initialized first (`git submodule update --init --recursive`).
